...
class ModelUserUser extends Model {
	...
	public function getUsersByFacilityUser($data) {
		if ($data ['facilities_id'] != null && $data ['facilities_id'] != "") {
			...
			if (! empty ( $data ['q'] )) {
				$sql2 .= " and LOWER(username) like '%" . strtolower ( $data ['q'] ) . "%'";
			}
	...
	public function insertUserOTP($data) {
		...
		if ($query->num_rows > 0) {
			...
			$user_otp_id = $query->row ['user_otp_id'];
		} else {
...
<table class="body-wrap" bgcolor="" style="width: 100%;    border-spacing: 0;">
	...
	public function getUsersByFacility1($data=array()) {
		...
		if ($data['facilities_id'] != null && $data['facilities_id'] != "") {
			...
			$sql2 .= " and CONCAT(LOWER(firstname), ' ', LOWER(lastname)) like '%" . strtolower ( $data ['username'] ) . "%'";
		   }

		  /* if (! empty ( $data ['username'] )) {
			$sql2 .= " and LOWER(lastname) like '%" . strtolower ( $data ['username'] ) . "%'";
		   }*/
	...
	public function getuserOPTbyid($user_id,$user_otp_id) {
		
		$sql = "SELECT * FROM `" . DB_PREFIX . "user_otp` WHERE user_id = '" . $user_id . "' and user_otp_id = '" . $user_otp_id . "' ";
		$query = $this->db->query ( $sql );
		return $query->row;
	}
	
	public function checkuserOPTbyid($user_id,$user_otp_id,$otp) {
		
		$sql = "SELECT * FROM `" . DB_PREFIX . "user_otp` WHERE user_id = '" . $user_id . "' and user_otp_id = '" . $user_otp_id . "' and otp = '" . $otp . "' ";
		$query = $this->db->query ( $sql );
		return $query->row;
	}
