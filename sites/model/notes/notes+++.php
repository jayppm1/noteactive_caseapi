...
class Modelnotesnotes extends Model {
		...
		if ($data ['activenote'] != null && $data ['activenote'] != "") {
			...
			if ($data ['activenote'] == 'all') {
				...
				if ($data ['relation_search'] == '1') {
					...
					echo 'keyword_image-'.$n_keyword_file = $keydata ['keyword_image'];
				}
			}
		}
		...
		if ($notes_descriptionstr != null && $notes_descriptionstr != "") {
			if ($facilities_id != null && $facilities_id != "") {
				if ($facilities_id != "0") {
					...
					$sqlxxx = "CALL insertNotesapp('" . $this->db->escape ( $notes_description ) . "','" . $highlighter . "','" . $this->db->escape ( $notes_pin ) . "','" . $data ['notes_file'] . "','" . $this->db->escape ( $username ) . "','" . $noteTime . "','" . $signature . "','" . $fileName . "','" . $this->db->escape ( $data ['text_color_cut'] ) . "','" . $this->db->escape ( $text_color ) . "','" . $createDate . "','" . $noteDate . "','" . $keyword_file . "','" . $keyword_icon . "','" . $highlighter_value1 . "','" . $this->db->escape ( $data ['emp_tag_id'] ) . "','" . $data ['tags_id'] . "','" . $this->db->escape ( $data ['notes_type'] ) . "','" . $update_date . "','" . $data ['offline'] . "','" . $data ['is_android'] . "', '" . $this->db->escape ( $data ['phone_device_id'] ) . "', '" . $this->db->escape ( $data ['device_unique_id'] ) . "','" . $facilities_id . "')";
					
					$lastId = $this->db->query ( $sqlxxx );
	...
	public function getAllNotebyactivenotes($data) {
		$sql = "SELECT count(t1.notes_by_keyword_id) AS total FROM `dg_notes_by_keyword` AS t1 INNER JOIN dg_notes AS t2 ON t1.notes_id = t2.notes_id WHERE 1=1";
		
		
		if ($data ['keyword_id'] != null && $data ['keyword_id'] != "") {
			$sql .= " and t1.keyword_id = '" . $data ['keyword_id'] . "' ";
		}
		
		if ($data ['user_id'] != null && $data ['user_id'] != "") {
			$sql .= " and t2.user_id = '" . $data ['user_id'] . "' ";
		}
