...
class Modelnotesnotes extends Model {
	public function addnotes($data, $facilities_id) {
			...
			foreach ($this->session->data ['multi_keyword_file'] as $key=>$multivalue ) {
				...
				foreach($multivalue as $v){
					...
					if($v['chkvalue'] != NULL && $v['chkvalue'] != ""){
						$sqla = "INSERT INTO `" . DB_PREFIX . "notes_by_multikeyword` SET notes_id = '" . $notes_id . "', keyword_id = '" . $key . "', facilities_id = '" . $facilities_id . "', date_added = '" . $noteDate . "', date_updated = '" . $noteDate . "', action = '" . $this->db->escape ( $v['action'] ) . "', name = '" . $this->db->escape ( $v['name'] ) . "', type = '" . $this->db->escape ( $v['measurement'] ) . "', value = '" . $this->db->escape ( $v['multivalue'] ) . "',unique_id = '" . $this->db->escape ( $unique_id ) . "' ";
						$this->db->query ( $sqla );
					}
				}
			}
		...
		if (! empty ( $data ['tagsids'] )) {
			...
			foreach ( $aabdcds as $tagid ) {
				...
				$tadata = array();
				$this->updateNotesTag ($tag_info ['emp_tag_id'], $notes_id, $tag_info['tags_id'], $update_date,$tadata );
			}
		}
		...
		if ($data ['tags_id'] != null && $data ['tags_id'] != "") {
			...
			$tadata = array();
			$this->updateNotesTag ( $data ['emp_tag_id'], $notes_id, $data ['tags_id'], $update_date,$tadata );
	...
	public function jsonaddnotes($data, $facilities_id) {
		...
		$sql = "CALL
          insertNotesapp('" . $this->db->escape ( $notes_description ) . "','" . $highlighter . "','" . $this->db->escape ( $notes_pin ) . "','" . $data ['notes_file'] . "','" . $this->db->escape ( $user_info ['username'] ) . "','" . $noteTime . "','" . $signature . "','" . $fileName . "','" . $this->db->escape ( $data ['text_color_cut'] ) . "','" . $this->db->escape ( $text_color ) . "','" . $createDate . "','" . $noteDate . "','" . $keyword_file . "','" . $keyword_icon . "','" . $highlighter_value1 . "','" . $this->db->escape ( $data ['emp_tag_id'] ) . "','" . $data ['tags_id'] . "','" . $this->db->escape ( $data ['notes_type'] ) . "','" . $update_date . "','" . $data ['offline'] . "','" . $data ['is_android'] . "', '" . $this->db->escape ( $data ['phone_device_id'] ) . "', '" . $this->db->escape ( $data ['device_unique_id'] ) . "','" . $facilities_id . "')";
		...
		if($data['updateMedication']!=null && $data['updateMedication']!=""){          
			$sqlUpdateMedication="UPDATE `" . DB_PREFIX . "notes` SET  is_forms = '" . $data['updateMedication'] . "' WHERE notes_id = '" . (int) $notes_id . "' ";
		...
		if ($data ['emp_tag_id'] != null && $data ['emp_tag_id'] != "") {
			...
			$sql = "update `" . DB_PREFIX . "notes_tags` set tag_status_id = '".$data['tag_status_id']."' ,tag_classification_id = '".$data['tag_classification_id']."',move_notes_id = '".$data['move_notes_id']."' where notes_tags_id='".$notes_tags_id."'";
			$this->db->query($sql);
	...
	public function getajaxnote($notes_id) {
		...
		if(is_array($notes_id)){
			...
			$update_medication_url = $this->url->link ( 'notes/addInventory/verifyInventory&medication_url=1', '' . $url2, 'SSL' );
			...
			$inventory_check_out_url = $this->url->link ( 'notes/addInventory/CheckOutInventory', '' . $url2, 'SSL' );

			$inventory_check_in_url = $this->url->link ( 'notes/addInventory/CheckInInventory', '' . $url2, 'SSL' );
			...
			foreach($notes_id2 AS $notes_row){
				...
				if($note['form_type'] == '7'){
					$notetime = date('h:i A',strtotime($note['uptimes']['notetime']));
				}else{
					$notetime = $note['notetime'];
				}
				...
				if ($note['form_type'] == '5' ) {           
					...
					$html .= '<a class="form_insert1"   href="' . $inventory_check_out_url . /*'&user_id=' . $note['is_inventory_checkout_id'] .*/ '&notes_id=' . $note['notes_id'] . ' ">';
					$html .= '<img src="sites/view/digitalnotebook/image/add_form.png" width="35px" height="35px" style=" ' . $csspadding . '">';
					$html .= '</a> ';
				}
				...
				if ($note['form_type'] == '6' ) {
					...
					$html .= '<a class="form_insert1"   href="' . $inventory_check_in_url . /*'&user_id=' . $note['is_inventory_checkin_id'] .*/ '&notes_id=' . $note['notes_id'] . ' ">';
					$html .= '<img src="sites/view/digitalnotebook/image/add_form.png" width="35px" height="35px" style=" ' . $csspadding . '">';
					$html .= '</a> ';
				}
				...
				if ($this->session->data ['unloack_success'] == '1') {
					if ($note ['is_tag'] != null && $note ['is_tag'] != "0") {
						if ($note ['form_type'] == '2') {
						 ...
						 if($note['form_type'] == '1' && $note['is_forms'] !="0"){

                         $medication_url=$update_medication_url;
	...
	public function getLastNotesID($facilities_id, $searchdate) {
		...
		$sql = "SELECT  * from `" . DB_PREFIX . "notes` WHERE facilities_id = '" . ( int ) $facilities_id . "' " . $sql . " ORDER BY notes_id DESC limit 0,1  ";
