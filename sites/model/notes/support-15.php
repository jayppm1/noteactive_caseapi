...
class Modelnotessupport extends Model {
	...
	public function addsupport($data, $facilities_id) {
		
		$sql = "INSERT INTO `" . DB_PREFIX . "support` SET facilities_id = '" . $facilities_id . "',user_id = '" . $data['user_id'] . "',support_by = '".$data['support_by']."',comment = '" . $data['comment'] . "',date_added = NOW() ";
	...
	public function getsupportbynumber($customer_number){
		$sql = "SELECT * FROM `" . DB_PREFIX . "support` WHERE customer_number = '" . $customer_number . "'";
		$query = $this->db->query($sql);
		return $query->row;
	}
