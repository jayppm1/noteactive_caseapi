...
class Modelresidentcasefile extends Model {
	...
	public function getcasefiles($data = array()) {
		if ($data ['tags_id'] != NULL && $data ['tags_id'] != "") {
			...
			if ($data ['casenumber'] != "") {
				$sql .= " and case_number = '" . $data ['casenumber'] . "' ";
			}
			...
			if ($data ['incident_type'] != "") {
				...
				//$sql .= " and incident_type = '" . $data ['incident_type'] . "' ";
			}
	...
	public function insertCasefile($data = array()){
		...
		foreach ( $unique_forms_ids as $val ) {
			$sqld = "UPDATE `" . DB_PREFIX . "forms` SET is_case = '1', date_updated = '" . $this->db->escape ( $date_added ) . "', case_number = '" . $data ['case_number'] . "' WHERE forms_id = '" . ( int ) $val . "'";
			$this->db->query ( $sqld );
		}
	...
	public function getCaseNumber($data = array()) {
		...
		//if ($data ['type'] != NULL && $data ['type'] != "") {
			...
			if ($data ['type'] == "casetype") {
				
				$sql .= "AND find_in_set(".$data ['value'].",case_type) ";
				
			}
			
			if ($data ['type'] == "incidenttype") {
				
				$sql .= "AND find_in_set(".$data ['value'].",incident_type) ";
				
			}
			
			if ($data ['type'] == "code") {
				
				$sql .= "AND find_in_set(".$data ['value'].",code) ";
				
			}
			
			if ($data ['type'] == "caseNumber") {
				
				$sql .= "AND case_number='" . $data ['value'] . "'";
			...
			if ($data ['type'] == "status") {
				...
				if($Casenumber['case_status']==0){
					...
					*/
					...
					if($data['value']=="Open"){
						$case_status='0';
					}elseif($data['case_status']=="Closed"){
						$case_status='1';
					}else{
						$case_status='2';
					}	
...
/*
	...
	public function deletecasecover($pdata) {
		...
		$facilities_id = $pdata ['facilities_id'];
		$timezone_name = $pdata ['facilitytimezone'];
		...
		$facilities_id3 = $pdata ['facilities_id'];
		
		$data ['notes_description'] = $pdata ['case_number'] . ' has been deleted form case' . $comments;
		
		// echo '<pre>'; print_r($data); print_r($case_info); echo '</pre>'; die;
		
		if ($pdata ['case_number'] != null && $pdata ['case_number'] != "") {
			$case_number = $pdata ['case_number'];
		} else {
		...
		$sql = "SELECT * FROM " . DB_PREFIX . "notes_by_case_file WHERE case_number = '" . $case_number . "' ";
			
		$query = $this->db->query ( $sql );
		$response = 0;
		if ($query->num_rows) {
			
			$delsql = "DELETE FROM `" . DB_PREFIX . "notes_by_case_file` WHERE case_number = '" . $case_number . "'";
			
			$response = $this->db->query ( $delsql );
			...
			/*$sql = "Update " . DB_PREFIX . "notes SET form_type = '9' WHERE notes_id = '" . $notes_id . "' ";
			$this->db->query ( $sql );
			
			if ($fdata ['notes_media_id'] != null && $fdata ['notes_media_id'] != "") {
				$sql = "UPDATE `" . DB_PREFIX . "notes_media`  SET case_number= '' WHERE notes_media_id='" . $fdata ['notes_media_id'] . "'";
				
				$this->db->query ( $sql );
			}*/
			/*
			if ($fdata ['forms_id'] != null && $fdata ['forms_id'] != "") {
				...
				$this->db->query ( $sql );
					...
					$sql = "UPDATE `" . DB_PREFIX . "notes_by_case_file`  SET forms_ids= '" . $forms_ids_str . "', notes_id= '" .$notes_id. "' WHERE notes_by_case_file_id='" . $fdata ['case_file_id'] . "'";
...
public function deletecasecover($pdata) {
		...
		if ($pdata ['notes_media_id'] != null && $pdata ['notes_media_id'] != "") {
			$sql = "UPDATE `" . DB_PREFIX . "notes_media`  SET case_number= '' WHERE notes_media_id='" . $pdata ['notes_media_id'] . "'";
			
			$this->db->query ( $sql );
		}
		
		if ($pdata ['forms_id'] != null && $pdata ['forms_id'] != "") {
			$sql = "UPDATE `" . DB_PREFIX . "forms`  SET is_case= '0' WHERE forms_id='" . $pdata ['forms_id'] . "'";
			
			$this->db->query ( $sql );
			
			$sql = "SELECT * FROM " . DB_PREFIX . "notes_by_case_file WHERE notes_by_case_file_id = '" . $pdata ['case_file_id'] . "' ";
			...
			if ($query->num_rows) {
				...
				$sql = "UPDATE `" . DB_PREFIX . "notes_by_case_file`  SET forms_ids= '" . $forms_ids_str . "', notes_id= '" .$notes_id. "' WHERE notes_by_case_file_id='" . $pdata ['case_file_id'] . "'";
				
				// echo $sql; die;
				$this->db->query ( $sql );
				
				$this->db->query ( "UPDATE `" . DB_PREFIX . "forms` SET is_case = '0', date_updated = '" . $this->db->escape ( $date_added ) . "' WHERE forms_id = '" . $pdata ['forms_id'] . "'" );
			}
			}
