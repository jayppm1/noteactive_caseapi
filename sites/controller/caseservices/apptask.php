	...
	public function jsonAddTasknote() {
		try {
			...
			*/
			...
			//echo '<pre>zzzz'; print_r($this->request->post); echo '</pre>'; die;
						...
						if ($this->request->post ['recurrence'] == 'daily') {
							...
							if ($this->request->post ['daily_times'] != null && $this->request->post ['daily_times'] != "") {
								...
								if ($current_time1 > $daily_endtime) {
									$json ['warning'] = "The Task Time must be greater than the Current Time zzzz";
									$facilitiessee = array ();
									$facilitiessee [] = array (
											'warning' => $json ['warning'] 
									);
							...
							if ($current_time1 > $taskTime) {
								$json ['warning'] = "The Task Time must be greater than the Current Time ggg";
								$facilitiessee = array ();
								$facilitiessee [] = array (
										'warning' => $json ['warning'] 
								);
			...
			if ($json ['warning'] == null && $json ['warning'] == "") {
				...
				if($this->request->post ['emp_tag_id1']=='' && $this->request->post ['emp_tag_id1']==NULL){
					...
					$emp_tag_id1 = $tag_info['emp_tag_id'].' : '.$tag_info['emp_first_name'].' '.$tag_info['emp_last_name'];
					$data ['emp_tag_id1'] = $emp_tag_id1;
				}else{
				...
				$exp_data = explode(',',$this->request->post ['assign_to']);
				
				$data ['assign_to'] = $exp_data;
