	...
	public function jsonCustomForms() {
		try {
			...
			foreach($results as $custom_form){
				$inmate_arr[] = $custom_form['forms_id'];
			}
			...
			if (! empty ( $results )) {
				...
				$value = array (
						'results' => $this->data ['facilitiess'],	'formsId' => $inmate_arr,
						'status' => true 
				);
				/* echo json_encode($value); */
				$this->response->setOutput ( json_encode ( $value ) );
					...
					if ($result ['is_forms'] == '1') {
						...
						foreach ( $allforms as $allform ) {
							...
							$forms [] = array (
									...
									'image_name' => $allform ['image_name'],
									'incident_number' => $allform ['incident_number'],
									'form_date_added' =>$form_date_added33,
									'href' => $form_url 
							)
							;
						}
...
;
			...
			$value = array (
					'results' => $this->data ['facilitiess'],
					'notesresults' => $this->data ['notesresults'],
					'Tasktotal' => $taskTotal,
					'status' => $error,
...
;
	...
	public function jsongetNoteByPageByClassification(){
		
		try {
			
			$this->data ['facilitiess'] = array ();
			$this->load->model ( 'activity/activity' );
			$this->model_activity_activity->addActivitySave ( 'jsongetNoteByPageByClassification', $this->request->post, 'request' );
			$json = array ();
			$this->load->model ( 'api/encrypt' );
			$cre_array = array ();
			$cre_array ['phone_device_id'] = $this->request->post ['phone_device_id'];
...
;
			...
			$this->load->model ( 'activity/activity' );
			$activity_data2 = array (
					'data' => 'Error in appservices jsongetNoteByPageByClassification ' . $e->getMessage () 
			);
			$this->model_activity_activity->addActivity ( 'app_jsongetNotesByPageByApp', $activity_data2 );
