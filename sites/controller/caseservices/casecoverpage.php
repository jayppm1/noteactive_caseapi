...
class Controllercaseservicescasecoverpage extends Controller {
	...
	public function jsonviewformlistbycase_number_and_case_id(){
		try {
			...
			$value = array('results'=>$json,'attachments'=>$attachments, 'status'=>true);
	...
	public function jsonaddcasecover(){
		try {
			...
			$client_name="";
			if($this->request->post['client_name']!="" ){
				$client_arrys = explode(',',$this->request->post['client_name']); 
			}
			
			
			//echo '<pre>jsonaddnotes'; print_r($data); echo '</pre>'; 
			//echo  $facilities_id;
			
			$tags_ids = array();
			foreach($client_arrys  as $client_arr){
					$tags_ids [] = array (
						'valueId' => $client_arr,
					);
	...
	public function jsontaggedformlist(){
		try{
			...
			$this->load->model ( 'form/form' );
			$allforms = $this->model_form_form->gettagsformslist ( $data );
			//echo '<pre>'; print_r($data); echo '</pre>'; //die;
			...
			$value = array('results'=>$results,'message'=> $message,'status'=>$status);
			$this->response->setOutput(json_encode($value));
	...
	public function jsonsavetaggedformtocase(){
		...
		try{
			...
			$notes_pin="";
			if($this->request->post['notes_pin']!=""){
				$notes_pin = $this->request->post['notes_pin'];
			}
			...
			$formData = json_decode(html_entity_decode($this->request->post['formsids']));
			
			$formsids="";
			$formsids_arr = array();	
			$formsname_arr = array();
			foreach($formData AS $erow){
				//echo $erow->action;
				//echo '<pre>'; print_r($erow); echo '<pre>'; 	
				$formsids_arr[] = $erow->action;
				$formsname_arr[] = $erow->form_name;
			}
			//die;
			
			
				
			/*if($this->request->post['formsids']!=""){
				$formsids = $this->request->post['formsids'];
				$formsids_arr = explode(',',$formsids);
			}
			
			if(is_array($formsids_arr)){
				foreach($formsids_arr AS $form_id){
					$res = $this->model_form_form->getFormDatas($form_id);
					$formsname_arr[] = $res['incident_number'];
				}
			}*/
		
			$form_name_list = @implode ( ',', $formsname_arr );
			$form_id_list = @implode ( ',', $formsids_arr );
			
			$pdata = array ();
			$pdata ['user_id'] = $user_id;
			$pdata ['notes_pin'] = $this->request->post['notes_pin'];
			$pdata ['comments'] = $comments;
			$pdata ['imgOutput'] = $imgOutput;
			...
			$this->load->model ( 'form/form' );	
			$notes_id = $this->model_form_form->caseFormListSign ( $pdata, $fdata );
			if($notes_id){
				$message = 'success';
				$status = true;
			}else{
			...
			$this->load->model('activity/activity');
			$activity_data = array(
				'data' => 'Error in apptask jsonsavetaggedformtocase '.$e->getMessage(),
			);
			$this->model_activity_activity->addActivity('app_jsonsavetaggedformtocase', $activity_data);
		}
	}
	...
	public function jsonsavetaggedformtocase(){
		...
		try{
			...
			$case_status="";
			if($this->request->post['status']!=""){
				$case_status = $this->request->post['status'];
			}
