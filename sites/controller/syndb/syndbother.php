<?php  
class Controllersyndbsyndbother extends Controller {
	...
	public function index() {
		...
		var_dump($facility['is_data_sync']);
		var_dump($facility['data_sync_date_from']);
		var_dump($facility['data_sync_date_to']);
		echo "<hr>";
		if($facility['is_data_sync'] == '1'){
			...
			if($manual_link == '1'){
				...
				if($facility['data_sync_date_from'] != "0000-00-00 00:00:00"){
					$startDate = $facility['data_sync_date_from'];
					$date_added_val = '1';
				}else{
					$startDate = date('Y-m-d', strtotime("-".DAY." day")) .' 00:00:00';
				}
				
				if($facility['data_sync_date_to'] != "0000-00-00 00:00:00"){
					$endDate = $facility['data_sync_date_to'];
				}else{
			...
			try{
				...
				if($date_added_val == '1'){
					...
					$sql = "SELECT * FROM `" . DB_PREFIX . "notes` where `update_date` BETWEEN  '".$startDate."' AND  '".$endDate."' and facilities_id='".$facilities_id."' ";
				}
				...
				if($notes != null && $notes != ""){
					...
					foreach($notes as $note){
						...
						$sqlg = "SELECT * FROM `" . DB_PREFIX . "notes` where `sync_records` = '".$note['notes_id']."' ";
						$queryg = $this->db->query($sqlg);
					
						$note_info = $queryg->row;
