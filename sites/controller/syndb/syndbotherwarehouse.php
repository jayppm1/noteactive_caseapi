<?php  
class Controllersyndbsyndbotherwarehouse extends Controller {
	
	public function __construct() {
		global $registry;
		parent::__construct($registry);
			$fnewdb = new DB(FNNEWDB_DRIVER1, FNNEWDB_HOSTNAME1, FNNEWDB_USERNAME1, FNNEWDB_PASSWORD1, FNNEWDB_DATABASE1);
			$registry->set('fnnewdbw', $fnewdb );
	...
	public function index() {
		...
		$connection = mysql_connect(FNNEWDB_HOSTNAME1, FNNEWDB_USERNAME1, FNNEWDB_PASSWORD1);
		var_dump($connection);
		echo mysql_error();
		mysql_select_db(FNNEWDB_DATABASE1, $connection);
		...
		/*$hostname = FNNEWDB_HOSTNAME1;
		$username = FNNEWDB_USERNAME1;
		$password = FNNEWDB_PASSWORD1;
		$dbname = FNNEWDB_DATABASE1;
		...
		*/
		...
		if($manual_link == '1'){
			...
			//$startDate = date('Y-m-d', strtotime("-".MONTH." Months")).' 00:00:00';
			//$endDate = date('Y-m-d');
			...
			if($notes != null && $notes != ""){
				...
				foreach($notes as $note){
					...
					/*	$sql3 = "SELECT * FROM `" . DB_PREFIX . "notes_by_task` where `notes_id` ='".$servernotes_id."' ";
						...
						$query4 = $this->fnnewdbw->query($sql4);
						...
						*/
						...
						$query5 = $this->fnnewdbw->query($sql5);
