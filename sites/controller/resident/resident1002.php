...
class Controllerresidentresident extends Controller
{
	...
	$this->data['roll_call_sign_url'] = str_replace('&amp;', '&', $this->url->link('common/authorization', '' . $url2, 'SSL'));
	  ...
	  $client_view_options_details = $client_info["client_details_view_options"];
       ...
       /* if($this->request->get['role_call']!="" && $this->request->get['role_call']!=null){
         //Total Role Call 
           ...
           $total_tags = $this->model_setting_tags->getTags($data31);
           $this->data['tags_total_2']=count($total_tags);
          ...
          //    var_dump($clientstatus_info);

          if($clientstatus_info['type']=='0' || $clientstatus_info['type']=='2'){
            ...
            $data31 = array(
             ...
             $tags_total_in = $this->model_setting_tags->getTags($data31);

             $this->data['tags_total']=count($tags_total_in);

             $this->data['total_out_tags']=count($total_tags)-count($tags_total_in);


            }else if($clientstatus_info['type']!='0' || $clientstatus_info['type']!='2'){
            ...
            $data31 = array(
             ...
             $tags_total_out = $this->model_setting_tags->getTags($data31);

             $this->data['total_out_tags']=count($tags_total_out);

             $this->data['tags_total']=count($total_tags)-count($tags_total_out);
        ...
        $tags = $this->model_setting_tags->getTags($data31);
         $this->data['tags_total_2']=count($tags);
       ...
       foreach ($tags as $tag) {
         ...
         if($tag['role_call']!=null && $tag['role_call']!=""){
            ...
            if($clientstatus_info['type']=='0' || $clientstatus_info['type']=="2"){
             ...
             $this->data['tags_total']=$i;

             $this->data['total_out_tags']=count($tags)-$i;             
       ...
       /* }     */
        ...
        foreach ($tags as $tag) 
		{
            ...
            if ($allform_info != null && $allform_info != "") {
			 ...
			 $med_data=$this->model_setting_tags->getMedicationCount($data);

            $med_count=$med_data['COUNT(`drug_name`)'];
     ...
     $this->load->model('form/form');
      ...
      if($customlistvalues_info['customlistvalues_name'] != null && $customlistvalues_info['customlistvalues_name'] != ""){
        ...
        if(isset($tag['facilities_id']) && $tag['facilities_id']!=''){
          $result_info = $this->model_facilities_facilities->getfacilities($tag['facilities_id']);
        $client_view_options_details2 = str_replace('[facilities_id]', $result_info['facility'], $client_view_options_details2); 
        } else{
        ...
        $client_view_options = str_replace('[room]', '', $client_view_options);
         ...
         if(isset($tag['room']) && $tag['room']!=''){
          $rresults = $this->model_setting_locations->getlocation($tag['room']);
        $client_view_options_details2 = str_replace('[room]', $rresults['location_name'], $client_view_options_details2);
        } else{
        ...
        $client_view_options = str_replace('[gender]', $customlistvalues_name, $client_view_options);
        } else{
        $client_view_options = str_replace('[gender]', '', $client_view_options);
        }

        if(isset($tag['gender']) && $tag['gender']!=''){  
        $client_view_options_details2 = str_replace('[gender]', $customlistvalues_name, $client_view_options_details2);
        } else{
      ...
      /*  if($tag['role_call']!='0' && $tag['role_call']!=null){*/
          ...
          'client_details' => nl2br($client_view_options_details2),
					...
					'status_type'=>$client_statuses_value['status_type'],
					'is_facility'=>$client_statuses_value['is_facility'],
					'facility_type'=>$client_statuses_value['facility_type'],
					'facility_move_id'=>$tag['facility_move_id'],
					'facility_inout'=>$tag['facility_inout'],
					
					  'tag_status_id'=>$client_statuses_value['tag_status_id'],
           
					'client_status_type'=>$client_statuses_value['type'], 
                    ...
                    'client_status_color'=>$client_statuses_value['color_code'],
                    'client_clssification_color'=>$classification_value['color_code'],
      ...
      /*  if($tag['role_call']!='0' && $tag['role_call']!=null){*/
          ...
          if($tag['tags_id']!='0' && $tag['tags_id']!=null){
            ...
            $json[] = array(
					...
					'name' => $tag['emp_first_name'] . ' ' . $emp_last_name,
					'facility' => $result_info['facility'],
					'default_facility_id'=>$facilities_id,					
					'name2' => nl2br($client_view_options),
          'client_view_flag'=> $client_view_options_flag,
				  'ssn'=>$tag['ssn'],   
          'client_details_view_flag'=>$client_details_view_flag,
        ...
        $add_tag_medication_url = str_replace('&amp;', '&', $this->url->link('resident/resident/tagsmedication', '', 'SSL'));
  ...
  public function multipleaction() {   
    ...
    $tag_ids = preg_split ("/\,/", $this->request->get['tags_ids']);
     ...
     if (array_unique($tags_ids_facilities) === array($this->request->get['default_facility_id'])) { 
...
*/
        ...
        foreach ($allforms as $allform) {
			...
			foreach($resultsforms as $resultsform){
				...
				$archivedforms[] = array(
						...
						'form_href' => $this->url->link('form/form&is_archive=4', '' . '&forms_id=' . $resultsform['forms_id'] . '&tags_id=' . $resultsform['tags_id'] . '&notes_id=' . $resultsform['notes_id'] . '&forms_design_id=' . $resultsform['custom_form_type'] . '&forms_id=' . $resultsform['forms_id'], 'SSL')
				);
   ...
   public function tagsmedicationsign ()
    ...
    {  
      ...
      $archive_tags_medication_id = $this->model_resident_resident->addTagsMedication($tempdata, $this->request->get['tags_id'],$this->request->get['updateMedication'], $facilities_id);
      ...
      $emp_tag_id = $tag_info['emp_last_name'] . ', ' . $tag_info['emp_first_name'] .' | '.$tag_info ['ssn'].' | '.$location_info ['location_name'].' | ';
        ...
        if($this->request->get['updateMedication']!=null && $this->request->get['updateMedication']!=""){
            ...
            $notes_description .= ' Medication Form updated | ';
             $data['updateMedication'] = $this->request->get['updateMedication'];
...
*/
    ...
    {   
      ...
      $facilities_id=""; $tag_status_id="";$name="";
  ...
  public function updateMedication ()
	...
	public function movementreport(){
		...
		if ($this->config->get('pdf_report_image') && file_exists(DIR_SYSTEM . 'library/pdf_class/'.$this->config->get('pdf_report_image'))) {
			...
			$PDF_HEADER_TITLE = "Movement Report";
	...
	';
		...
		<table width="100%" style="border:none;" cellpadding="2" cellspacing="0" >
			<tr>  
				<td width="20%" style="text-align:center;"><img src="'.HTTPS_SERVER.'image/Lafayette-logo.png" style="padding:20px;text-align:center;height:100px;" > </td>
				<td width="80%" style="text-align:center;">
				<h2>CORRECTIONS DIVISION</h2>
				<P>Lafayette Parish Sheriff\'s Office<br>
				<b>Lafayette Parish Correctional Center (LPCC)</b><br>
				Facility Destination Log</P>
				</td>
