...
class Controllerresidentresident extends Controller {
	...
	public function allclientstatus() {
		...
		if (($this->request->server ['REQUEST_METHOD'] == 'POST')) {
			...
			if ($this->request->post ['parent_ids'] != null && $this->request->post ['parent_ids'] != "") {
				$url2 .= '&parent_ids=' . $this->request->post ['parent_ids'];
			}
			
			if ($this->request->get ['increment'] != null && $this->request->get ['increment'] != "") {
				$url2 .= '&increment=' . $this->request->get ['increment'];
			}
		...
		$this->data ['substatuspopup_url'] = str_replace ( '&amp;', '&', $this->url->link ( 'resident/resident/substatuspopup', '', 'SSL' ) );
		...
		if ($this->request->get ['type'] == 2) {
			...
			$data3 ['role_call'] = $customers ['defaultrole_call'];
		}
		$data3 ['out_from_cell'] = 2;
		...
		if ($this->request->get ['tag_status_name'] != null && $this->request->get ['tag_status_name'] != "") {
			$url2 .= '&name=' . $this->request->get ['tag_status_name'];
		}
		...
		foreach ( $client_statuses as $custom_form ) {
			...
			$this->data ['custom_forms'] [] = array (
					...
					'parent_ids' => $custom_form ['parent_ids'],
		...
		$data3 ['status'] = '1';
		$data3 ['facilities_id'] = $facilities_id;
		$data3 ['out_from_cell'] = 1;
		$data3 ['display_client'] = "";
		
		$sclient_statuses = $this->model_resident_resident->getClientStatus ( $data3 );
		
		$this->data ['specialcustom_forms'] = array ();
		foreach ( $sclient_statuses as $sclient_status ) {
			
			$rule_action_content = unserialize ( $sclient_status ['rule_action_content'] );
			...
			$this->data ['specialcustom_forms'] [] = array (
					'tag_status_id' => $sclient_status ['tag_status_id'],
					'name' => $sclient_status ['name'],
					'parent_ids' => $sclient_status ['parent_ids'],
					'type' => $sclient_status ['type'],
					'image' => $sclient_status ['image'],
					'formhref' => $href 
			);
		}
		// }
	...
	public function StatusCount() {
		...
		if ($this->request->get ['type'] == 2) {
			$this->data ['status_wise_url'] = $this->url->link ( 'resident/resident/hourout&role_call=' . $this->request->get ['tag_status_id'], '', 'SSL' );
		} else {
	...
	public function allclientstatuses() {
		...
		if ($this->request->get ['tags_ids'] != null && $this->request->get ['tags_ids'] != "") {
			...
			$userIds21 = explode ( ',', $this->request->get ['tags_ids'] );
			
			if(count($userIds21) == 1){
				$taginfo = $this->model_setting_tags->getTag ( $this->request->get ['tags_ids'] );
				
				if(!empty($taginfo)){
					$url2 .= "&tags_id=" . $taginfo['tags_id'];
				}else{
					$this->data ['hide_status'] = 1;
				}
			}else{
				$this->data ['hide_status'] = 1;
			}
		...
		$this->data ['substatuspopup_url'] = str_replace ( '&amp;', '&', $this->url->link ( 'resident/resident/substatuspopup&multipleclient=1', '', 'SSL' ) );
	...
	protected function validateForm23() {
		...
		/*if ($this->request->get ['tag_status_id'] != '' && $this->request->get ['tag_status_id'] != null) {
			...
			if(!empty($ruleaction_content['status_user_roleids'])){
				if (!in_array($user_info ['user_group_id'], $ruleaction_content['status_user_roleids'] )) {
					$this->error ['warning'] = "You are not authorized to change status!";
				}
			}
		}
		*/
		...
		if ($this->request->get ['tag_status_id'] != '' && $this->request->get ['tag_status_id'] != null) {
			...
			if(!empty($tags_info1)){
				if($tags_info1['fixed_status_id'] > 0){
					$clientstatus_info = $this->model_notes_clientstatus->getclientstatus ($tags_info1['fixed_status_id']);
				}else{
					$clientstatus_info = $this->model_notes_clientstatus->getclientstatus ($tags_info1['role_call']);
				}
				...
				if(!empty($ruleaction_content['status_user_roleids'])){
					$changenewuids = array();
					foreach($ruleaction_content['status_user_roleids'] as $userroleid){
						if($ruleaction_content[$userroleid]['change_status'] == '1'){
							$changenewuids[] = $userroleid;
						}
					}
					
					if (!in_array($user_info ['user_group_id'], $changenewuids)) {
						$this->error ['warning'] = "You are not authorized to reset ".$clientstatus_info['name']." status!";
					}
				}
			}
		}
		...
		if ($tag_status_info ['disabled_escorted'] == '0') {
			if ($facility ['enable_escorted'] == '1') {
				if ($facility ['required_escorted'] == '1') {
	...
	public function masterfacility() {
		...
		foreach ( $mfacilities as $mfacility ) {
			...
			if ($this->request->get ['redirectpage'] == "hourout") {
				$facilityurl = str_replace ( '&amp;', '&', $this->url->link ( 'resident/resident/hourout&search_facilities_id=' . $mfacility ['facilities_id'], '', 'SSL' ) );
			} else {
				$facilityurl = str_replace ( '&amp;', '&', $this->url->link ( 'resident/resident&search_facilities_id=' . $mfacility ['facilities_id'], '', 'SSL' ) );
			}
	...
	public function movementreport() {
		...
		';
			...
			$html .= '<td style="text-align:left; line-height:20.2px;width:20%">' . $description1 . '';
			
			if ($manual_movement == '1') {
				$html .= '<br> Manual Movement';
			}
	...
	public function hourout() {
		...
		if ($this->request->get ['search_facilities_id'] > 0) {
			$this->session->data ['search_facilities_id'] = $this->request->get ['search_facilities_id'];
			$this->redirect ( $this->url->link ( 'resident/resident/hourout', '' . $url2, 'SSL' ) );
		}
		...
		if (($this->request->get ['searchtag'] == '1')) {
			...
			$this->redirect ( $this->url->link ( 'resident/resident/hourout', '' . $url, 'SSL' ) );
		}
		...
		if ($this->request->get ['search_filter'] != null && $this->request->get ['search_filter'] != "") {
			$this->data ['search_filter'] = $this->request->get ['search_filter'];
		}
		...
		if ($this->request->get ['search_tags'] != null && $this->request->get ['search_tags'] != "") {
			$url4 .= '&search_tags=' . $this->request->get ['search_tags'];
			$url5 .= '&search_tags=' . $this->request->get ['search_tags'];
		}
		
		if ($this->request->get ['room_id'] != null && $this->request->get ['room_id'] != "") {
			$url4 .= '&room_id=' . $this->request->get ['room_id'];
			$url5 .= '&room_id=' . $this->request->get ['room_id'];
		}
		
		if ($this->request->get ['wait_list'] != null && $this->request->get ['wait_list'] != "") {
			$url4 .= '&wait_list=' . $this->request->get ['wait_list'];
			$url5 .= '&wait_list=' . $this->request->get ['wait_list'];
		}
		
		
		if ($this->request->get ['search_tags_tag_id'] != null && $this->request->get ['search_tags_tag_id'] != "") {
			$url4 .= '&search_tags_tag_id=' . $this->request->get ['search_tags_tag_id'];
			$url5 .= '&search_tags_tag_id=' . $this->request->get ['search_tags_tag_id'];
		}
		
		if ($this->request->get ['gender'] != null && $this->request->get ['gender'] != "") {
			$url4 .= '&gender=' . $this->request->get ['gender'];
			$url5 .= '&gender=' . $this->request->get ['gender'];
		}
		
		if ($this->request->get ['add_role_call'] != null && $this->request->get ['add_role_call'] != "") {
			$url4 .= '&add_role_call=' . $this->request->get ['add_role_call'];
			$url5 .= '&add_role_call=' . $this->request->get ['add_role_call'];
		}
		
		if ($this->request->get ['role_call'] != null && $this->request->get ['role_call'] != "") {
			$url4 .= '&role_call=' . $this->request->get ['role_call'];
			$url5 .= '&role_call=' . $this->request->get ['role_call'];
		}
		
		if ($this->request->get ['client_status'] != null && $this->request->get ['client_status'] != "") {
			$url4 .= '&client_status=' . $this->request->get ['client_status'];
			$url5 .= '&client_status=' . $this->request->get ['client_status'];
		}
		
		if ($this->request->get ['sort'] != null && $this->request->get ['sort'] != "") {
			$url4 .= '&sort=' . $this->request->get ['sort'];
			$url5 .= '&sort=' . $this->request->get ['sort'];
		}
		if ($this->request->get ['order'] != null && $this->request->get ['order'] != "") {
			$url4 .= '&order=' . $this->request->get ['order'];
			$url5 .= '&order=' . $this->request->get ['order'];
		}
		
		if ($this->request->get ['search_filter'] != null && $this->request->get ['search_filter'] != "") {
			$url4 .= '&search_filter=' . $this->request->get ['search_filter'];
		}
		
		// // $this->data['male_url'] = $this->url->link('resident/resident&gender=1', '' . $url1, 'SSL');
		// $this->data['female_url'] = $this->url->link('resident/resident&gender=2', '' . $url1, 'SSL');
		
		$this->data ['total_in_url'] = $this->url->link ( 'resident/resident/hourout&in=1', '' . $url1, 'SSL' );
		$this->data ['total_out_url'] = $this->url->link ( 'resident/resident/hourout&out=1', '' . $url1, 'SSL' );
		$this->data ['non_url'] = $this->url->link ( 'resident/resident&/houroutgender=3', '' . $url1, 'SSL' );
		
		$this->data ['total_url'] = $this->url->link ( 'resident/resident/hourout', '', 'SSL' );
		
		$this->data ['notes_url'] = $this->url->link ( 'notes/notes/insert/hourout', '', 'SSL' );
		...
		$this->data ['resident_url'] = str_replace ( '&amp;', '&', $this->url->link ( 'resident/resident/hourout', '' , 'SSL' ) );
		$this->data ['residenturl'] = str_replace ( '&amp;', '&', $this->url->link ( 'resident/resident/hourout', '' . $url5, 'SSL' ) );
		$this->data ['task_lists'] = str_replace ( '&amp;', '&', $this->url->link ( 'notes/createtask/headertasklist', '' . $url1, 'SSL' ) );
		...
		$this->data ['status_wise_url'] = $this->url->link ( 'resident/resident/StatusCount', '', 'SSL' );
		
		$this->data ['classification_wise_url'] = $this->url->link ( 'resident/resident/ClassificationCount', '', 'SSL' );
		$default_facility_id = $this->model_facilities_facilities->getfacilities ( $this->session->data ['search_facilities_id'] );
		...
		if ($this->request->get ['search_filter'] != "" && $this->request->get ['search_filter'] != null) {
			...
			foreach ( $customforms as $customform ) {
				...
				if ($this->request->get ['search_filter'] == 1) {
					if ($customform ['type'] == "0" || $customform ['type'] == "2") {
						$inclint [] = $customform ['tag_status_id'];
					}
				}
				
				if ($this->request->get ['search_filter'] == 2) {
					

					if ($customform ['out_from_cell'] == "1") {
						$ruleaction_ontent = unserialize ( $customform ['rule_action_content'] );
						//if($ruleaction_ontent['forms_id'] == null && $ruleaction_ontent['forms_id'] == ""){
							$outcount [] = $customform ['tag_status_id'];
				...
				if ($this->request->get ['search_filter'] == 3) {
					if ($customform ['out_from_cell'] == "1") {
						$ruleaction_ontent = unserialize ( $customform ['rule_action_content'] );
						if($ruleaction_ontent['forms_id'] == null && $ruleaction_ontent['forms_id'] == ""){
							$filters1 [] = $customform ['tag_status_id'];
						}
					}
				}

				if ($this->request->get ['search_filter'] == 4) {
					if ($customform ['out_from_cell'] == "1") {
						$ruleaction_ontent = unserialize ( $customform ['rule_action_content'] );
		...
		if ($filters1 != null && $filters1 != "") {
			$filters12 = implode ( ",", $filters1 );
			$filters = $filters12;
		}
		...
		foreach ( $tags as $tag ) {
			...
			// var_dump( $tag ['location_name']);die;
			...
			if ($ruleaction_content ['out_from_cell'] == 1) {
				...
				foreach ( $notesmedicationtimes1 as $notesmedicationtime ) {
					...
					$statusimages [] = $notesmedicationtime ['image'];
					$notesmedicationtimes [] = array (
		...
		$this->data ['clientstatuses_url'] = str_replace ( '&amp;', '&', $this->url->link ( 'resident/resident/allclientstatuses&type=2', '' . $url2, 'SSL' ) );
		
		$this->data ['clientstatus_url'] = str_replace ( '&amp;', '&', $this->url->link ( 'resident/resident/allclientstatus&type=2', '' . $url2, 'SSL' ) );
		
		$this->data ['multiple_action_url'] = str_replace ( '&amp;', '&', $this->url->link ( 'resident/resident/multipleaction&type=2', '' . $url2, 'SSL' ) );
		...
		$this->data ['action'] = str_replace ( '&amp;', '&', $this->url->link ( 'resident/resident/hourout', '', 'SSL' ) );
		...
		if ($this->request->get ['search_filter'] != null && $this->request->get ['search_filter'] != "") {
			$url .= '&search_filter=' . $this->request->get ['search_filter'];
		}
		...
		$this->data ['sorts'] [] = array (
				'text' => 'Default/Lastname',
				'value' => 'emp_last_name',
				'href' => $this->url->link ( 'resident/resident/hourout', '' . '&sort=emp_last_name&order=ASC' . $url3 ) 
		);
		
		$this->data ['sorts'] [] = array (
				'text' => 'Firstname',
				'value' => 'emp_first_name',
				'href' => $this->url->link ( 'resident/resident/hourout', '' . '&sort=emp_first_name&order=ASC' . $url3 ) 
		);
		
		$this->data ['sorts'] [] = array (
				'text' => 'Location',
				'value' => 'l.location_name',
				'href' => $this->url->link ( 'resident/resident/hourout', '' . '&sort=l.location_name&order=ASC' . $url3 ) 
		);
		
		$this->data ['sorts'] [] = array (
				'text' => 'Updated Date',
				'value' => 'modify_date',
				'href' => $this->url->link ( 'resident/resident/hourout', '' . '&sort=modify_date&order=ASC' . $url3 ) 
		);
		
		$this->data ['sorts'] [] = array (
				'text' => 'Intake Date',
				'value' => 'date_added',
				'href' => $this->url->link ( 'resident/resident/hourout', '' . '&sort=date_added&order=ASC' . $url3 ) 
		);
		...
		if ($this->request->get ['add_role_call'] != '1') {
			...
			$pagination->limit = $config_admin_limit;
			$pagination->text = $this->language->get ( 'text_pagination' );
			$pagination->url = $this->url->link ( 'resident/resident/hourout', 'page={page}' . $url, 'SSL' );
	...
	public function ajaxresidenthourout() {
		...
		$clientstatus_url = str_replace ( '&amp;', '&', $this->url->link ( 'resident/resident/allclientstatus&type=2', '' . $url2, 'SSL' ) );
		...
		$template->data ['activenote_url'] = $activenote_url;
		$template->data ['is_external_status_facility'] = $facilities_is_master ['enable_facilityinout'];
		$template->data ['select_all'] = $this->request->get ['select_all'];
		$template->data ['page'] = $this->request->get ['page'];
	...
	public function allhouroutpopup(){
		...
		if (($this->request->post ['form_submit'] == '1')) {
			...
			if ($this->request->post ['search_time_start'] != null && $this->request->post ['search_time_start'] != "") {
				$url2 .= '&search_time_start=' . $this->request->post ['search_time_start'];
			}
			
			if ($this->request->post ['search_time_to'] != null && $this->request->post ['search_time_to'] != "") {
				$url2 .= '&search_time_to=' . $this->request->post ['search_time_to'];
			}
			
			if ($this->request->post ['shift_id'] != null && $this->request->post ['shift_id'] != "") {
				$url2 .= '&shift_id=' . $this->request->post ['shift_id'];
			}
			
			if ($this->request->post ['tags_ids'] != null && $this->request->post ['tags_ids'] != "") {
				
				$tags_ids = implode(",",$this->request->post ['tags_ids']);
				
				$url2 .= '&tags_ids=' . $tags_ids;
			}
			
			if ($this->request->post ['statuses'] != null && $this->request->post ['statuses'] != "") {
				
				$status_ids = implode(",",$this->request->post ['statuses']);
				
				$url2 .= '&status_ids=' . $status_ids;
			}
		...
		if ($this->request->get ['search_time_start'] != null && $this->request->get ['search_time_start'] != "") {
			...
			if ($this->request->get ['shift_id'] != null && $this->request->get ['shift_id'] != "") {
				$url2 .= '&shift_id=' . $this->request->get ['shift_id'];
			}
			...
			if ($this->request->get ['status_ids'] != null && $this->request->get ['status_ids'] != "") {
				
				$status_ids = $this->request->get ['status_ids'];
		...
		$customer_data = array ();
		$customer_data ['facilities_id'] = $facilities_id;
		$customer_data ['customer_key'] = $customer_info['activecustomer_id'];
		
		$shift_info = $this->model_notes_notes->getAllShift ( $customer_data );
	...
	public function allhouroutreport() {
		...
		foreach ( $tags as $tag ) {
			$outcelltimtime = $this->secondsToTime($tag ['status_total_time']*60);
			$json [$tag ['ssn']]['name'] = $tag ['emp_last_name'] . ', ' . $tag ['emp_first_name'];
			
			$json [$tag ['ssn']][date ( 'm-d-Y', strtotime ($tag ['Event_Date']))][] = array (
				'name' => $tag ['emp_last_name'] . ', ' . $tag ['emp_first_name'],
				'ssn' => $tag ['ssn'],
				'room' => $tag ['room'],
				'race' => $tag ['race'],
				'location_name' => $tag ['location_name'],
				'status_total_time' => $outcelltimtime,
				'comments' => $tag ['comments'],
				'status_name1' => $tag ['status_name1'],
				'status_name2' => $tag ['status_name2'],
				'tag_status_ids' => $tag ['tag_status_ids'],
				'Customlistvalues' => $tag ['Customlistvalues'],
				'Event_Date' => date ( $date_format, strtotime ($tag ['Event_Date'])),
				'notesintime' => date ($time_format, strtotime ($tag ['NotesInTime'])),
				'notesouttime' => date ($time_format, strtotime ($tag ['NotesOutTime'])),
			);
	...
	';
		...
		foreach ( $json as $atag ) {
			$html .= ' <table width="100%" style="border:none;background-color: #000; color: #fff;" cellpadding="2" cellspacing="0" >
				<tr>  
					<td width="100%" style="padding:20px;text-align:left;width:100%;padding: 10px;margin: 10px; border: 1px solid #B8b8b8; line-height:20.2px;display:table-cell;padding-bottom: 8px;padding-top: 8px;" > ' . $atag ['name'] .'</td>
				</tr>
			</table>';
			
			foreach ( $atag as $keydate=>$atag1 ) {
				if($keydate != 'name'){
					$html .= ' <table width="100%" >
						<tr>  
							<td width="100%" style="background: #eee; color: #fa801b;padding-bottom: 8px;padding-top: 8px;">&nbsp;&nbsp;&nbsp;&nbsp; ' . $keydate .'</td>
						</tr>
					</table>';
				
					foreach($atag1 as $outcelltim){ 
						$html .= ' <table width="100%" style="border:none;" cellpadding="2" cellspacing="0" >';
						$html .= '<tr> ';
						
						$html .= '<td width="20%">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;' . $outcelltim['status_name1'] . '</td>';
						
						$html .= '<td width="20%">' . $outcelltim ['status_name2'] . ' ' . $outcelltim ['tag_status_ids'] . ' ' . $outcelltim ['Customlistvalues'] . ' ' . $outcelltim ['comments'] . '</td>';
						$html .= '<td width="20%">' . $outcelltim['Event_Date'] . '</td>';
						$html .= '<td width="15%">' . $outcelltim ['notesouttime'] . '</td>';
						$html .= '<td width="15%">' . $outcelltim ['notesintime'] . '</td>';
						$html .= '<td width="10%">' . $outcelltim ['status_total_time'] . '</td>';
	...
	public function substatuspopup() {
		$this->load->model ( 'resident/resident' );
		$this->load->model ( 'setting/tags' );
		$this->load->model ( 'facilities/facilities' );
		$timezone_name = $this->customer->isTimezone ();
		...
		if ($this->request->get ['parent_ids'] != null && $this->request->get ['parent_ids'] != "") {
			$url2 .= '&parent_ids=' . $this->request->get ['parent_ids'];
		}
		
		if ($this->request->get ['substatus'] != null && $this->request->get ['substatus'] != "") {
			$url2 .= '&substatus=' . $this->request->get ['substatus'];
		}
		if ($this->request->get ['multipleclient'] != null && $this->request->get ['multipleclient'] != "") {
			$url2 .= '&multipleclient=' . $this->request->get ['multipleclient'];
		}
		
		if (($this->request->server ['REQUEST_METHOD'] == 'POST') && $this->validateForm24 ()) {
			
			foreach ( $this->request->post ['statusmulti'] as $key => $val ) {
				// var_dump($key);
				if ($val ['chkvalue'] == 1) {
					$tag_status_ids_arr [] = $val ['name'];
				}
			}
			
			$this->session->data ['statusmulti'] = $this->request->post ['statusmulti'];
			
			if (! empty ( $tag_status_ids_arr )) {
				$tag_status_ids_str = implode ( ',', $tag_status_ids_arr );
				
				$url2 .= '&substatus_ids=' . $tag_status_ids_str;
			}
			...
			$this->data ['authorization'] = 1;
		}
		
		if ($this->request->get ['tag_status_id'] != NULL && $this->request->get ['tag_status_id'] != "") {
			$clientstatus = $this->model_resident_resident->getClientStatusById ( $this->request->get ['tag_status_id'] );
			$status_ids = unserialize ( $clientstatus ['parent_ids'] );
			$rule_action_content = unserialize ( $clientstatus ['rule_action_content'] );
			
			$this->data ['selectstatus_value'] = $rule_action_content ['selectstatus_value'];
			
			foreach ( $status_ids as $statusid ) {
				
				// $client_status = $this->model_resident_resident->getClientStatusById ( $status_id );
				$this->data ['multiplesrow'] [] = array (
						'tag_status_id' => $statusid ['name'],
						'is_comments' => $statusid ['is_comments'],
						'comment_required' => $statusid ['comment_required'],
						'name' => $statusid ['name'] 
				);
			}
			
			// echo '<pre>'; print_r($this->data ['multiplesrow']); echo '</pre>';
		}
		...
		if ($this->request->get ['tag_status_id'] != NULL && $this->request->get ['tag_status_id'] != "") {
			$this->data ['tag_status_id'] = $this->request->get ['tag_status_id'];
		} else {
			$this->data ['tag_status_id'] = '';
		}
		
		if ($this->request->get ['substatus'] != null && $this->request->get ['substatus'] != "") {
			$this->data ['substatus'] = $this->request->get ['substatus'];
		}
		if ($this->request->get ['multipleclient'] != null && $this->request->get ['multipleclient'] != "") {
			$this->data ['multipleclient'] = $this->request->get ['multipleclient'];
		}
		
		$this->data ['action2'] = $this->url->link ( 'resident/resident/substatuspopup', '' . $url2, 'SSL' );
		
		// echo '<pre>'; print_r($this->data); echo '</pre>';
		
		$this->template = $this->config->get ( 'config_template' ) . '/template/resident/multiple.php';
	...
	public function validateForm24() {
		if ($this->request->post ['form_key'] != null && $this->request->post ['form_key'] != "") {
			$formkeyerror = $this->formkey->validate ( $this->request->post ['form_key'] );
		}
		...
		/*
		 * $mchk = '';
		 * foreach ( $this->request->post ['chkvalue'] as $key => $value ) {
		 * $mchk .= $value;
		 * }
		 *
		 * if ($mchk == "" || $mchk == NULL) {
		 * $this->error ['warning'] = "Please select at least one status";
		 * }
		 */
		
		$mchk = '';
		foreach ( $this->request->post ['statusmulti'] as $key => $value ) {
			
			if ($value ['is_comments'] == "1") {
				if ($value ['comment_required'] == "1") {
					if ($value ['chkvalue'] == '1') {
						// if($v['multivalue'] == "")
						
						$data = trim ( $value ['multivalue'] );
	...
	public function ClassificationCount() {
		if ($this->request->get ['facilities_id'] != null && $this->request->get ['facilities_id'] != "") {
		...
		$this->data ['classification_wise_url'] = $this->url->link ( 'resident/resident&role_classification=' . $this->request->get ['tag_classification_id'], '', 'SSL' );
		...
		$this->data ['classification_forms'] = array ();
		foreach ( $client_classifications as $custom_form ) {
			
			$this->data ['classification_forms'] [] = array (
					'tag_classification_id' => $custom_form ['tag_classification_id'],
					'name' => $custom_form ['classification_name'] 
			);
		}
		...
		$this->template = $this->config->get ( 'config_template' ) . '/template/resident/ClassificationCount.php';
		$this->children = array (
				'common/headerpopup' 
		);
		$this->response->setOutput ( $this->render () );
