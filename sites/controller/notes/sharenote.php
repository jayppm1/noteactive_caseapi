...
class Controllernotessharenote extends Controller {
	...
	public function addnote() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if($facility_info['is_sms_enable'] == '1'){
				...
				if($userdetail['phone_number'] == null && $userdetail['phone_number'] == ""){
					$this->error['warning'] = "Please contact your admin !";	
				}else{
					...
					$dataotp = array(
						...
						'alternate_email' => $this->request->post['user_email'],
						'share_note_otp' => $share_note_otp,
						'status' => '1',
						'otp_type' => 'shared_note',
					);
					
					
					$url = '';
					$url .= '&user_id='.$userdetail['user_id'];
					$url .= '&share_note_otp='.$share_note_otp;
					$url .= '&otp_type=shared_note';
		...
		$edata1['assignto_email'] = $assignto_email;	
		$edata1['message'] = $message33;	
		
			$edata1['type'] = "12";
			$edata1['username'] = $this->request->post['user_id'];
			$edata1['who_user'] = $this->request->post['user_id'];
			$edata1['when_date'] = date("l");
			
			$edata1['subject'] = 'Share Notes';
		...
		$this->template = $this->config->get('config_template') . '/template/notes/sharenote.php';
	...
	protected function validateForm(){
		...
		if($this->request->post['user_id'] != NULL && $this->request->post['user_id'] != ""){
			...
			if($userdetail['email'] == NULL && $userdetail['email'] == ""){
				...
				$this->error['warning'] = "Selected user don't have email address, please select another user";
...
return $html;
	...
	public function searchnoteshare(){
		...
		*/
		...
		if($this->session->data['note_date_to'] != null && $this->session->data['note_date_to'] != ""){
			...
			$date2 = str_replace('-', '/', $this->session->data['note_date_to']);
			$res2 = explode("/", $date2);
			$changedDate1 = $res2[1]."-".$res2[0]."-".$res2[2];
			
			$note_date_to = date('Y-m-d', strtotime($changedDate1));
		}
		...
		$searchresulthtml .='<thead>';
			...
			$searchresulthtml .='    <td valign="middle" style="text-align: left;background-color: #DAEEF3;width:30%">Note Description</td>';
			...
			$searchresulthtml .='    <td valign="middle" style="text-align: right;background-color: #DAEEF3;width:15%">Download Link</td>';
			...
			$searchresulthtml .='<td style="line-height:20.2px;width:30%;text-align:left;'.$cssStyle.'">';
			...
			$searchresulthtml .='<td style="text-align:right;width:15%; line-height:20.2px;">';  
			...
			if($allforms){
				foreach($allforms as $forms){
					if($forms['custom_form_type'] == '9'){
						$searchresulthtml .='<a href="'.$firedrillnoteurl.'&notes_id='.$result['notes_id'].'&forms_design_id='.$forms['custom_form_type'].'&forms_id='.$forms['forms_id'].'" target="_blank"><img src="'.HTTP_SERVER.'sites/view/digitalnotebook/image/add_form.png" width="35px" height="35px" ></a> ';
					}
					
					if($forms['custom_form_type'] == '13'){
						$searchresulthtml .='<a href="'.$printnoteurl.'&notes_id='.$result['notes_id'].'&forms_design_id='.$forms['custom_form_type'].'&forms_id='.$forms['forms_id'].'" target="_blank"><img src="'.HTTP_SERVER.'sites/view/digitalnotebook/image/add_form.png" width="35px" height="35px" ></a> ';
					}
					
					if($forms['custom_form_type'] == '10'){
						$searchresulthtml .='<a href="'.$incidentnoteurl.'&notes_id='.$result['notes_id'].'&forms_design_id='.$forms['custom_form_type'].'&forms_id='.$forms['forms_id'].'" target="_blank"><img src="'.HTTP_SERVER.'sites/view/digitalnotebook/image/add_form.png" width="35px" height="35px" ></a> ';
					}
					
					if($forms['custom_form_type'] == '2'){
						$searchresulthtml .='<a href="'.$innoteurl.'&notes_id='.$result['notes_id'].'&forms_design_id='.$forms['custom_form_type'].'&forms_id='.$forms['forms_id'].'" target="_blank"><img src="'.HTTP_SERVER.'sites/view/digitalnotebook/image/add_form.png" width="35px" height="35px" ></a> ';
					}
		...
		$edata1['message'] = $message33;
		$edata1['dirpath'] = $dirpath;
		$edata1['filename'] = $searchfilename;
		$edata1['assignto_email'] = $assignto_email;	
	...
	public function searchnotepage(){
		...
		$this->template = $this->config->get('config_template') . '/template/notes/sharenotepage.php';
