...
class Controllernotescreatetask extends Controller {
	private $error = array ();
   	public function index() {
		try {
			...
			$this->data ['facilities'] = $this->model_facilities_facilities->getfacilitiess ($data3);
			...
			if($this->data ['tasktypes']['0']['is_facility']=="1"){

			  $this->data ['facility_type']=$this->data ['tasktypes']['0']['facility_type'];
			...
			if (isset ( $this->request->post ['taskTime'] )) {
				...
				$this->data ['taskTime'] = date ($timeinfo['time_format'], strtotime ( ' +3 minutes', strtotime ( 'now' ) ) );
			}
			if (isset ( $this->request->post ['endtime'] )) {
				$this->data ['endtime'] = $this->request->post ['endtime'];
			} else {
				$this->data ['endtime'] = date ( $timeinfo['time_format'], strtotime ( ' +60 minutes', strtotime ( 'now' ) ) );
			}
            ...
            if (isset ( $this->request->post ['facilities'] )) {
				$facilities = $this->request->post ['facilities'];
			}  else {
				$facilities = array ();
			}
			
			$this->data ['totalfacilities'] = array ();
			$this->load->model ( 'facilities/facilities' );
			
			foreach ( $facilities as $facility_id ) {
				
				$facility_info = $this->model_facilities_facilities->getfacilities ( $facility_id );
			...
			if (isset ($this->request->post ['facility_type'])) {

			if ($this->request->post ['facility_type']=="1") {	
				$this->data ['facilities_sub_id'] = $this->request->post ['facility_type'];
			} else{

				$this->data ['facilities_sub_id'] = '';

			}
		    }		   	

			if (isset ( $this->request->post ['facility_type'] )) {
				if ($this->request->post ['facility_type']=="0") {
				$this->data ['facilities_id'] = $this->request->post ['facility_type'];
			} else {
				$this->data ['facilities_id'] = '';
			}
		    }

		   // var_dump($this->data ['facilities_id']." ".$this->data ['facilities_sub_id']);die;
						...
						foreach ( $medication_tags1 as $medicationtag ) {
							...
							if ($tags_info1) {
								...
								if ($tags_info1 ['emp_tag_id'] != null && $tags_info1 ['emp_tag_id'] != "") {
									$this->load->model ( 'notes/notes' );
									$tadata = array();
									$this->model_notes_notes->updateNotesTag ( $tags_info1 ['emp_tag_id'], $notes_id, $tags_info1 ['tag_id'], $update_date,$tadata );
								}
							}
						}
					}
...

	{
		...
		if ($this->request->get ['task_id'] != '') {
			...
			$tasktype_info = $this->model_createtask_createtask->gettasktyperowByName ( $result ['tasktype'],$this->customer->getId () );
			...
			if ($this->request->post ['perpetual_checkbox'] == '2') {
				if($tasktype_info['pause_user_roles'] != null && $tasktype_info['pause_user_roles'] != ""){
					...
					if (!in_array ( $user_info['user_group_id'], $user_roles3 )) {
						$this->error ['warning'] = "You are not authorized to Pause on this task!";
			...
			if ($this->request->post ['perpetual_checkbox'] == '3') {
				if($tasktype_info['move_user_roles'] != null && $tasktype_info['move_user_roles'] != ""){
					...
					if (!in_array ( $user_info['user_group_id'], $user_roles4 )) {
						$this->error ['warning'] = "You are not authorized to Move on this task!";
			...
			if ($this->request->post ['perpetual_checkbox'] == '4') {
				if($tasktype_info['changeinterval_user_roles'] != null && $tasktype_info['changeinterval_user_roles'] != ""){
					...
					if (!in_array ( $user_info['user_group_id'], $user_roles6 )) {
						$this->error ['warning'] = "You are not authorized to Change Interval on this task!";
	...
	public function inserttask() {       
		...
		try {
			...
			if ($this->request->get ['task_id'] != Null && $this->request->get ['task_id'] != "") {
					...
					$notes_id = $this->model_createtask_createtask->inserttask ( $result, $this->request->post, $facilities_idt, $this->request->get ['requires_approval'] );
					...
					if ($this->request->post ['perpetual_checkbox'] == '1') {
						...
						$notesid = $this->model_notes_notes->jsonaddnotes ( $data, $facilities_idt );
	...
	public function updateInCompleteTask() {
		try {
				...
				if ($complteteTaskLists != null && $complteteTaskLists != "") {
					...
					foreach ( $complteteTaskLists as $complteteTaskList ) {
						...
						if ($tasktype_info ['auto_extend'] == '0') {
							...
							$facilities_id = $complteteTaskList['facilityId'];
			...
			$this->load->model ( 'activity/activity' );
			$activity_data2 = array (
					'data' => 'Error in Sites Updateincompletetask ' .$e->getMessage()
			);
	...
	public function approvalurl() {
		...
		if ($task_ino ['is_approval_required_forms_id'] > 0) {
			...
			if ($form_info ['user_id'] != null && $form_info ['user_id'] != "") {
				...
				if ($form_info ['form_date_added'] != null && $form_info ['form_date_added'] != "0000-00-00 00:00:00") {
					$form_date_added = date ( $timeinfo['date_format'], strtotime ( $form_info ['form_date_added'] ) );
				} else {
					$form_date_added = '';
				}
			} else {
				...
				if ($note_info ['note_date'] != null && $note_info ['note_date'] != "0000-00-00 00:00:00") {
					$form_date_added = date ( $timeinfo['date_format'], strtotime ( $note_info ['note_date'] ) );
				} else {
					$form_date_added = '';
				}
			}
		...
		foreach ( $approvaltasklists as $list ) {
			...
			$this->data ['listtask'] [] = array (
					...
					'pickup_facilities_id' => $list ['pickup_facilities_id'],
					'pickup_locations_address' => $list ['pickup_locations_address'],
					'pickup_locations_time' => date ( $timeinfo['time_format'], strtotime ( $list ['pickup_locations_time'] ) ),
					'pickup_locations_latitude' => $list ['pickup_locations_latitude'],
					'pickup_locations_longitude' => $list ['pickup_locations_longitude'],
					'dropoff_facilities_id' => $list ['dropoff_facilities_id'],
					'dropoff_locations_address' => $list ['dropoff_locations_address'],
