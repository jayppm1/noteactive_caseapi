...
class Controllerservicesresident extends Controller { 
		...
		try{
			...
			foreach ($tags as $tag) {
				...
				if($tag['privacy'] == '2'){
				   ...
				   $tasksinfo = $this->model_createtask_createtask->getTaskas($tag['tags_id']);
				...
				$taskTotal = $this->model_createtask_createtask->getCountTasklist($this->request->post['facilities_id'], $changedDate, $top, $facilitytimezone, $tag['tags_id']);
			...
			$this->data['offsitetags_total'] = $this->model_setting_tags->getTotalTags($data6);
			...
			$this->data['dischargetags_total'] = $this->model_setting_tags->getTotalTags($data7);
			...
			$this->data['totaltags_total'] = $this->model_setting_tags->getTotalTags($data8);
			*/
			$datai = array();
			$datai = array(
				'status' => 1,
	...
	public function jsonAddCensus(){
		...
		if($json['warning'] == null && $json['warning'] == ""){
			...
			$sql = "UPDATE `" . DB_PREFIX . "notes` SET is_census = '1' where notes_id = '" . (int)$notes_id . "' ";
			$this->db->query($sql);
		...
		$this->response->setOutput(json_encode($value));
			...
			$this->load->model('activity/activity');
			$activity_data2 = array(
				'data' => 'Error in addcencus jsonAddNotes',
			);
	...
	public function addclient(){
		...
		if (($this->request->post['form_submit'] == '1') && $this->validateForm()) {
			...
			if($this->request->post['emp_tag_id'] != null && $this->request->post['emp_tag_id'] != ""){
				...
				$this->model_setting_tags->editTags($this->request->post['emp_tag_id'], $this->request->post);
	...
	public function updateclient(){
		...
		if (($this->request->post['form_submit'] == '1') && $this->validateForm()) {
			...
			$this->load->model('setting/tags');
			$this->model_setting_tags->editTags($this->request->get['tags_id'], $this->request->post);
	...
	protected function validateForm() {
		...
		if ((utf8_strlen($this->request->post['emp_first_name']) < 1) || (utf8_strlen($this->request->post['emp_first_name']) > 32)) {
			$this->error['emp_first_name'] = "This is required field!";
		}

		if ((utf8_strlen($this->request->post['emp_last_name']) < 1) || (utf8_strlen($this->request->post['emp_last_name']) > 32)) {
		...
		if ($this->request->post['emp_first_name'] != '') {
	...
	protected function getForm() {
		...
		if (isset($this->request->post['gender'])) {
			...
			$this->data['gender'] = $taginfo['gender'];
		} else {
			$this->data['gender'] = '';
		}
	...
	public function jsoncustomsForm(){
			...
			if ($this->request->get['notes_id'] != null && $this->request->get['notes_id'] != "") {
				...
				$signature_url = $this->url->link('services/resident/insert2', '' . $url2, 'SSL');
			}else{
				$notes_id = '';
				$signature_url = $this->url->link('services/resident/insert2', '' . $url2, 'SSL');
			}
	...
	public function tagsmedication(){
		...
		if (($this->request->post['form_submit'] == '1') && $this->validateForms()) {
			...
			$this->model_resident_resident->addTagsMedication($this->request->post, $tags_id);
		...
		if (isset($this->request->post['new_module'])) {
			...
			$muduled = $this->model_resident_resident->gettagModule($this->request->get['tags_id']);
			
			$this->data['modules'] = $muduled['new_module'];
		}elseif ($this->request->post['emp_tag_id']) { 
			
			$muduled = $this->model_resident_resident->gettagModule($this->request->post['emp_tag_id']);
		...
		if (isset($this->request->post['medication_fields'])) {
			...
			$medicine_info = $this->model_resident_resident->gettagmedicine($this->request->get['tags_id']);
			
			$this->data['medication_fields'] = unserialize($medicine_info['medication_fields']);
		}elseif ($this->request->post['emp_tag_id']) { 
			
			$medicine_info = $this->model_resident_resident->gettagmedicine($this->request->post['emp_tag_id']);
	...
	public function tagsmedicationsign2(){
		...
		if($json['warning'] == null && $json['warning'] == ""){
			...
			if($this->request->get['facilities_id']){
				...
				$sql = "UPDATE `" . DB_PREFIX . "notes` SET form_type = '1', is_tag = '" . (int)$this->request->get['tags_id'] . "' where notes_id = '" . (int)$notes_id . "' ";
				$this->db->query($sql);
	...
	public function tagsmedicationsign(){
		...
		if($json['warning'] == null && $json['warning'] == ""){
			...
			$description .= ' Meditation given to | ';
			$description .= ' '. $medication_tags;
			...
			if($this->request->get['medication_tags']){
				...
				foreach ($medication_tags1 as $medicationtag) {
					...
					if ($mdrug_info) {
						...
						$sql2 = "INSERT INTO `" . DB_PREFIX . "notes_by_task` SET notes_id = '".$notes_id."', locations_id ='".$mdrug_info['locations_id']."', task_type= '2', task_content = '".$this->db->escape($task_content)."', signature= '".$mdrug_info['medication_signature']."', user_id= '".$mdrug_info['medication_user_id']."', date_added = '".$date_added."', notes_pin = '".$mdrug_info['medication_notes_pin']."', notes_type = '".$this->request->post['notes_type']."', task_time = '".$mdrug_info['task_time']."' , media_url = '".$mdrug_info['media_url']."', capacity = '".$mdrug_info['capacity']."', location_name = '".$mdrug_info['location_name']."', location_type = '".$mdrug_info['location_type']."', notes_task_type = '2', tags_id = '".$tag_info['tags_id']."', drug_name = '".$mdrug_info['drug_name']."', dose = '".$mdrug_info['dose']."', drug_type = '".$mdrug_info['drug_type']."', quantity = '".$mdrug_info['quantity']."', frequency = '".$mdrug_info['frequency']."', instructions = '".$mdrug_info['instructions']."', count = '".$mdrug_info['count']."', createtask_by_group_id = '".$mdrug_info['createtask_by_group_id']."', task_comments = '".$mdrug_info['comments']."', medication_attach_url = '".$mdrug_info['medication_attach_url']."',medication_file_upload='1' , tags_medication_details_id = '".$mdrug_info['tags_medication_details_id']."' , tags_medication_id = '".$mdrug_info['tags_medication_id']."'  ";
	...
	public function jsoncustomsForm2(){
			...
			if ($this->request->get['addmedicine'] == "2") {
				$medform_url = $this->url->link('services/resident/tagsmedicationsign2', '' . $url2, 'SSL');
			}else{
				$medform_url = $this->url->link('services/resident/tagsmedicationsign', '' . $url2, 'SSL');
			}
		...
		if($json['warning'] == null && $json['warning'] == ""){
			...
			if($this->request->post['facilities_id']){
				...
				if($this->request->post['role_calls']){
					...
					$data['notes_description'] = $keywordData2['keyword_name'].' | '. $tagname .' | '.$boygirl.' Clients are in the Facility '. $outtag . $description1.$comments;
	...
	public function jsonclienttagform(){
		...
		$results = $this->model_form_form->gettagsforms($tags_id);
		...
		foreach ($results as $allform) {
			...
			if($allform['user_id'] != null && $allform['user_id'] != ""){
				...
				if($allform['custom_form_type'] == '13' ){
						$print_url = $this->url->link('form/form/printform', $url2, true);
					}else
					if($allform['custom_form_type'] == '9' ){
					...
					if($allform['custom_form_type'] == '10' ){
		...
		$value = array('results'=>$this->data['facilitiess'],'status'=>true, 'client_name'=>$name);
