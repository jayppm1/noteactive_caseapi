...
class Controllerservices2resident extends Controller { 
	private $error = array();
		...
		try{
		   ...
		   // var_dump($unique_id); die;
			...
			foreach ($tags as $tag) {
				...
				$tasktypes = $this->model_createtask_createtask->getTaskdetails($this->request->post['facilities_id']);
				...
				foreach($tasktypes as $tasktype){
					...
					$taskTotal1 = $this->model_createtask_createtask->getCountTasklist($this->request->post['facilities_id'], $changedDate, $top, $facilitytimezone, $tag['tags_id'], $tasktype['task_id']);
	...
	public function jsonClientListCommon(){
		try{
		   ...
		   // var_dump($unique_id); die;
			...
			$this->request->post['facilities_id'] = '108';
			$this->load->model('facilities/facilities');
			$facilityinfo = $this->model_facilities_facilities->getfacilities($this->request->post['facilities_id']);
			$this->load->model('notes/notes');
	...
	public function jsonClientListTags(){
		...
		try{
		 ...
		 //var_dump(config_case_limit); 
			if ($this->config->get ( 'config_case_limit' ) != null && $this->config->get ( 'config_case_limit' ) != "") {
				$config_case_limit = $this->config->get ( 'config_case_limit' );
			} else {
				$config_case_limit = "50";
			}
			...
			foreach ($tags as $tag) {
			  ...
			  if($client_view_options != "" && $client_view_options != null){
				...
				$this->data['tags'][] = array(		
					...
					'upload_file' => $upload_file_thumb_1 ? $upload_file_thumb_1 : "",
					'privacy' => $tag['privacy'],
	...
	protected function getForm() {
		...
		if (isset($this->request->post['imageName_url'])) {
			...
			if($get_img['upload_file_thumb'] != null && $get_img['upload_file_thumb'] != ""){
				...
				$this->data['imageName_url'] = $get_img['upload_file'];
			}
			$this->load->model('setting/image');
		...
		if($json['warning'] == null && $json['warning'] == ""){
			...
			if($this->request->post['facilities_id']){
				...
				if($this->request->post['is_switch'] == "1"){
					...
					$data['notes_description'] = $taskcontent . ' | ' . $form_name . ' status changed to | ' . $roleCall2  . $description1 . $comments;
				...
				if($this->request->post['role_call']){
					...
					$data['notes_description'] = $taskcontent .' status changed '.$caltime.' to | '.$roleCall2.$description1. $comments ;
				...
				if(!empty($role_calltagsids)){
					...
					foreach($role_calltagsids as $facilities_id=>$rolecalls){
						...
						if($rolecall['discharge'] == '1'){
							...
							$data['notes_description'] = $keywordData2['keyword_name'].' | '. $tagname .''.$description1.$comments;
						}else{
							$data['notes_description'] = $tagname . ' status changed '. $caltime .$roleCallname .$description1. $comments ;	
						}
			...
			*/
			...
			foreach ( $results as $allform ) {
				...
				$this->data ['facilitiess'] [] = array (
						...
						'date_added2' => date ( 'm-d-Y', strtotime ( $allform ['date_added'] ) ),
						'display_date' => date ( 'D F j, Y', strtotime ( $allform ['date_added'] ) ),
						'href' => $form_url,
						'print_url' => $print_url,
	...
	/*  public function jsonclienttagform(){
		...
		/* 
		$this->data['clientdetail'][] = array(
		 'client_name'=>$name,
		 'add_client_url' => $this->data['add_client_url'], 
		 'add_tag_medication_url' => $this->data['add_tag_medication_url']
		 );
		...
		$value = array('results'=>$this->data['facilitiess'],'form_total' => $form_total,'status'=>true, 
		 'atagsforms' => $this->data['atagsforms'], 'displayforms' => $this->data['displayforms'], 'client_name'=>$name );
	...
	*/
	...
	$value = array('results'=>$this->data['filterkeywords'],'keywordsTotal' => $keywordsTotal, 'Lastpage' => ceil($keywordsTotal/$config_admin_limit),'status'=>true);
	$this->response->setOutput(json_encode($value));
