...
class Controllerservices2apptask extends Controller {
		...
		try {
			...
			if ($facilities_id != null && $facilities_id != "") {
				...
				if ($facilities_info ['config_task_status'] == '1') {
					...
					$listtasks = $this->model_createtask_createtask->getTasklist ( $facilities_id , $currentdate, $top, '');
			...
			$value = array (
					...
					'Tasktotal' => $taskTotal,
					'Lastpage' => ceil($taskTotal/$config_admin_limit),
					'status' => true 
			);
...
//tags_medication_details_ids
			...
			*/ 
			...
			if ($json ['warning'] == null && $json ['warning'] == "") {
				...
				;
					...
					//["1269"" string(16) ""1270"" string(23) ]
					//["1240"" string(17) ""1260"]
				
				
				
				//var_dump($this->request->post ['tags_medication_details_ids']);
				
				//var_dump(explode('parent-',$this->request->post ['tags_medication_details_ids']));
				
				//json_decode($this->request->post ['tags_medication_details_ids']);
				
				
				
				
				$jsonData = stripslashes ( html_entity_decode ( $this->request->post ['tags_medication_details_ids'] ) );
				$tags_medication_details_ids = json_decode ( $jsonData, true );
				
				
				//$tags_medication_details_ids = explode(',',$this->request->post ['tags_medication_details_ids']);
				
				
				//var_dump($id);
				
				$tags_medication_details_result = array();
				
					foreach($tags_medication_details_ids as $ids){
						
						$id = explode('parent-',$ids);
						if($id[0] == ''){
							//$tags_medication_details_result[] = '';
							continue;
						}else{
							$tags_medication_details_result[] = $id;
				...
				$data ['tags_medication_details_ids'] = $tags_medication_details_result;
				...
				if($this->request->post['facilities_id']!=null && $this->request->post['facilities_id']!=""){
					$facilities = explode(",",$this->request->post['facilities_id']);
				...
				foreach ( $results as $info ) {
					...
					if (! empty ( $tagsmedications )) {
						$this->data ['tagmedicationData'] [] = array (
								...
								'tagmedication_display' => $info ['emp_tag_id'] . ': ' . $info ['emp_first_name'] . ' ' . $info ['emp_last_name'],
								'tagsmedications' => $this->model_setting_tags->getTagsMedicationdetails ( $info ['tags_id'] ) 
						)
						;
					}
			...
			if ($json ['warning'] == null && $json ['warning'] == "") {
				...
				if ($tasklocations != null && $tasklocations != "") {
					foreach ( $tasklocations as $tasklocation ) {
						...
						$fre_array2c ['facilities_id'] = $this->request->post ['facilities_id'];
	...
	public function jsonSavetaskBymedication() {
		try {
			...
			if ($this->request->post ['task_id'] != '') {
				 ...
				 * $json['warning'] = "Inventory is showing that the required QTY has been exhausted.Please update the Inventory.";
				 * $facilitiessee = array();
				 * $facilitiessee[] = array(
				 * 'warning' => $json['warning'],
				 * );
	...
	public function jsongetcustomelist() {
		try {
			...
			https://demo.noteactive.com/index.php?route=services/apptask/jsongetcustomelist
			*/
	...
	public function tagmedication(){
			...
			*/
			...
			foreach($tag as $t){
				...
				$results = $this->model_setting_tags->getTagsMedicationdetails ($t);
				$tags_info1 = $this->model_setting_tags->getTag ( $t );
				$rs = $this->model_api_permision->getclientinfo ( $facilities_id, $tags_info1 );
				
				//var_dump($results);
				/*
					$datamedication[] =  array( 
								'id' => $t, 
								'name'=> $rs['name'],
								'hasChild'=> 'true' );
					*/			
					...
					if (! empty ( $results )) {
						...
						foreach ( $results as $result ) {
							...
							$datamedication[] = array( 
								'id' => $result['tags_medication_details_id'], 
								'pid' => 'parent-'.$t,  
					...
					*/
					...
					$value = array (
							//'results' => $this->data ['facilitiess'],
							'results' => $datamedication,
							'status' => true 
					);
					/* echo json_encode($value); */
					$this->response->setOutput ( json_encode ( $datamedication ) );
				} else {
					$this->data ['facilitiess'] [] = array (
