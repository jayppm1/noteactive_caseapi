...
class Controllerservicesappservices extends Controller {
	public function jsonFacilities() {
		try {
			...
			} else if ($this->request->get ['is_web'] == "1") {
				$results = $this->model_facilities_facilities->getfacilitiess ( $data2 );
			} else {
		...
		try {
			...
			if ($facility_info ['facilities_id'] != null && $facility_info ['facilities_id'] != "") {
				...
				if ((empty ( $results )) && ($users == null && $users == "")) {
					$json ['warning'] = 'Please check Facility mapping with Users';
					$facilitiessee = array ();
					$facilitiessee [] = array (
							'warning' => $json ['warning'] 
					);
	...
	public function jsonAddNotes() {
		try {
			...
			if ($json ['warning'] == null && $json ['warning'] == "") {
				...
				if ($this->request->post ['role_call'] != null && $this->request->post ['role_call'] != "") {
					...
					$substatus_ids = json_decode ( $jsonData, true );
					
					foreach ( $substatus_ids as $key => $val ) {
				...
				if ($this->request->post ['is_web'] == 1) {
					...
					$facilitiesids = array ();
					if ($_REQUEST ['facilities_ids'] != null && $_REQUEST ['facilities_ids'] != "") {
						$facilitiesids1 = explode ( ",", $_REQUEST ['facilities_ids'] );
						
						foreach ( $facilitiesids1 as $facilid ) {
							$facilitiesids [] = array (
									'valueId' => $facilid 
							);
						}
					...
					$locations = array ();
					if ($_REQUEST ['location_ids'] != null && $_REQUEST ['location_ids'] != "") {
						$location_ids1 = explode ( ",", $_REQUEST ['location_ids'] );
						
						foreach ( $location_ids1 as $locationid ) {
							$locations [] = array (
									'valueId' => $locationid 
							);
						}
					...
					$userids = array ();
					if ($_REQUEST ['user_ids'] != null && $_REQUEST ['user_ids'] != "") {
						$userids1 = explode ( ",", $_REQUEST ['user_ids'] );
				...
				if (! empty ( $clients )) {
					foreach ( $clients as $client ) {
						...
						if ($client ['facilityId'] != null && $client ['facilityId'] != "") {
							$facilityId1 = $client ['facilityId'];
						} else {
							$facilityId1 = $tag_info ['facilities_id'];
						}
						
						$aids [$facilityId1] ['clients'] [] = array (
				...
				if (! empty ( $locations )) {
					foreach ( $locations as $location ) {
						...
						if ($location ['facilityId'] != null && $location ['facilityId'] != "") {
							...
							$facilityId = $location_info12 ['facilities_id'];
						}
				...
				if (! empty ( $facilitiesids )) {
					foreach ( $facilitiesids as $facilitiesid ) {
						...
						if ($facilitiesid ['facilityId'] != null && $facilitiesid ['facilityId'] != "") {
							$facilityId12 = $facilitiesid ['facilityId'];
						} else {
							$facilityId12 = $facilityinfo ['facilities_id'];
						}
						
						$aids [$facilityId12] ['facilitiesids'] [] = array (
								'valueId' => $facilitiesid ['valueId'] 
						);
					}
				}
				if (! empty ( $multifacilities )) {
						...
						if ($facilities_id != null && $facilities_id != "") {
							...
							if ($aid ['clients'] != null && $aid ['clients'] != "") {
								...
								if ($data ['role_call'] != null && $data ['role_call'] != "") {
									...
									$rstatusname = ' Status changed to | ' . $roleCallname . $statusname;
								}
	...
	public function jsonupdateText() {
		try {
			...
			if ($this->request->post ['notes_id'] != null && $this->request->post ['type'] == 'text') {
				...
				if ($this->request->post ['is_web'] == '1') {
					$this->model_notes_notes->updateNoteColorwebang ( $this->request->post ['notes_id'], $this->request->post ['text_color'], $update_date );
				} else {
	...
	public function jsonKeywordsurl() {
		try {
			...
			if (! empty ( $results )) {
					...
					if ($result ['multiples_module'] != null && $result ['multiples_module'] != "") {
						...
						foreach ( $taskmodules as $taskmodule ) {
							$modules [] = array (
									...
									'facilitydata_type' => $taskmodule ['facilitydata_type'],
									'facilitydefault_value' => $taskmodule ['facilitydefault_value'],
				...
				$this->data ['facilitiess'] [] = array (
						'warning' => "Please check Facility mapping with ActiveNote" 
				);
				$error = false;
