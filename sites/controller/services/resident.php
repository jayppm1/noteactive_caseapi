...
class Controllerservicesresident extends Controller {
		...
		try {
			...
			if (! empty ( $tags )) {
				foreach ( $tags as $tag ) {
					...
					if (isset ( $tag ['bed_number'] ) && $tag ['bed_number'] != '') {
							
						$client_view_options = str_replace ( '[bed_number]', $tag ['bed_number'], $client_view_options );
					} else {
						$client_view_options = str_replace ( '[bed_number]', '', $client_view_options );
					}
						
					if (isset ( $tag ['bed_number'] ) && $tag ['bed_number'] != '') {
							
						$client_view_options_details2 = str_replace ( '[bed_number]', $tag ['bed_number'], $client_view_options_details2 );
					} else {
						$client_view_options_details2 = str_replace ( '[bed_number]', '', $client_view_options_details2 );
					}
		...
		try {
			...
			foreach ( $customforms as $customform ) {
				...
				foreach($parentids as $parentid){
					if($parentid ['is_comments'] != null && $parentid ['is_comments'] != ""){
						$is_comments = $parentid ['is_comments'];
					}else{
						$is_comments = 0;
					}
					if($parentid ['comment_required'] != null && $parentid ['comment_required'] != ""){
						$comment_required = $parentid ['comment_required'];
					}else{
						$comment_required = 0;
					}
					$parent_ids [] = array (
				...
				if ($rule_action_content ['forms_id'] != '' && $rule_action_content ['forms_id'] != null) {
					$href = str_replace ( '&amp;', '&',$this->url->link ( 'services/form', '' . '&forms_design_id=' . $rule_action_content ['forms_id'] . '&tag_status_id=' . $customform ['tag_status_id'] . $url2, 'SSL' ));
				} else {
					$href = "";
				}
				...
				$this->data ['clientstatuss'] [] = array (
						...
						'status_type' => $customform ['status_type'],
						'is_facility' => $customform ['is_facility'],
						'out_from_cell' => $customform ['out_from_cell'],
			...
			if (! empty ( $tags )) {
				foreach ( $tags as $tag ) {
					...
					if ($tag ['fixed_status_id'] > 0) {
						...
						$statusimages [] = array('image'=>$fixclientstatus_info ['image']);
						...
						if ($ruleaction_ontent ['forms_id'] > 0) {
							...
							$forminfo = $this->model_form_form->getFormDatabynotesbyid ( $ruleaction_ontent ['forms_id'], $tag ['tags_id'] );
							
							$fixstatusformdate = date ( $date_format, strtotime ( $forminfo['date_added'] ) );
							
							$fixstatusformhref = str_replace ( '&amp;', '&', $this->url->link ( 'services/form', '' . '&forms_design_id=' . $ruleaction_ontent ['forms_id'] . '&forms_id=' . $forminfo['forms_id'] . '&notes_id=' . $forminfo['notes_id'].  $url2, 'SSL' ));
						}
					}
		...
		try {
			...
			if ($json ['warning'] == null && $json ['warning'] == "") {
				...
				if ($this->request->post ['is_switch'] == "1") {
					...
					$data ['notes_description'] = $taskcontent . ' | ' . $form_name . ' status updated to | ' . $roleCall2 . $description1 . $comments. $statusname;
				...
				if ($this->request->post ['facilities_id'] != null && $this->request->post ['facilities_id'] != "") {
					...
					$jsonData = stripslashes ( html_entity_decode ( $this->request->post['substatus_ids']) );
					$substatusids = json_decode ( $jsonData, true );
				
					foreach ( $substatusids as $key => $val ) {
						
						if($val['value'] != null && $val['value'] != ""){
							$status_name [] = $val['name'].' - '.$val['value'];
						}else{
							$status_name [] = $val['name'];
						}
						
						if($val['value'] != null && $val['value'] != ""){
							$multivalue [] = $val['value'];
						}
					...
					if ($clientstatus_info2 ['type'] == '4') {
						...
						$data ['notes_description'] = $roleCall2 . " completed | " . $taskcontent . ' ' . $fname . ' | ' . $roominfo ['location_name'] . ' ' . $caltime . $description1 . $comments.$statusname;
					} else {
						$data ['notes_description'] = $taskcontent . ' status updated ' . $caltime1 . ' to | ' . $roleCall2 . $caltime . $description1 . $comments.$statusname;
					}
					...
					if($rule_action_content['exclude_in_inmate_status'] != null && $rule_action_content['exclude_in_inmate_status'] != ""){
						...
						$fixed_status_id = $this->request->post ['role_call'];
					}else{
						$tag_status_id = $this->request->post ['role_call'];
					...
					$data ['out_from_cell'] = $clientstatus_info2 ['out_from_cell'];
					
					if ($rule_action_content ['exclude_in_inmate_status'] != null && $rule_action_content ['exclude_in_inmate_status'] != "") {
						if ($rule_action_content ['forms_id'] == "" && $rule_action_content ['forms_id'] == null) {
							$data ['refused_status_id'] = $fixed_status_id;
							$data ['refused_tag_status_ids'] = $substatus_ids;
							$data ['refused_comments'] = $substatus_idscomment;
						}else{
							$data ['fixed_status_id'] = $fixed_status_id;
							$data ['substatus_ids'] = $substatus_ids;
							$data ['substatus_idscomment'] = $substatus_idscomment;
					...
					$cdata2 ['facility_inout'] = $this->request->post ['facility_inout'];
					$cdata2 ['substatus_ids'] = $substatus_ids;
					...
					$this->model_resident_resident->updatetagrolecall ( $this->request->post ['tags_id'], $tag_status_id );
					$this->load->model ( 'api/facerekognition' );
					$fre_array2 = array ();
					$fre_array2 ['face_notes_file'] = $this->request->post ['face_notes_file'];
					$fre_array2 ['outputFolder'] = $this->request->post ['outputFolder'];
					...
					foreach ( $role_calltagsids as $facilities_id => $rolecalls ) {
						if($facilities_id != null && $facilities_id != ""){
							...
							foreach ( $rolecalls as $rolecall ) {
								...
								foreach ( $rolecall ['substatus_ids'] as $key => $val ) {
								...
								if ($rolecall ['discharge'] == '1') {
									...
									if ($clientstatus_info2 ['type'] == '4') {
										...
										$data ['notes_description'] = $tagname . ' status updated ' . $caltime1 . $roleCallname . $caltime . $description1 . $comments.$comments1.$statusname;
									}
								}
								...
								if($rule_action_content['exclude_in_inmate_status'] != null && $rule_action_content['exclude_in_inmate_status'] != ""){
									...
									$fixed_status_id = $rolecall ['role_call'];
								}else{
									$tag_status_id = $rolecall ['role_call'];
				...
				if (! empty ( $facility_inouts )) {
					...
					foreach ( $facility_inouts as $facilities_id => $facility_inout1 ) {
						...
						foreach ( $facility_inout1 as $facility_inout2 ) {
							...
							$data ['notes_description'] = $tagname . ' | ' . $form_name . ' status updated to ' . $roleCall . $description1 . $comments;
	...
	public function jsonClientoutofcellgraphs() {
		try {
			...
			$alltotal = $this->model_setting_tags->getTotalTags ( $datat4 );
			$tags = $this->model_setting_tags->getTags ( $datat4 );
			...
			$activity_data2 = array (
					'data' => 'Error in resident jsonClientoutofcellgraphs ' . $e->getMessage () 
			);
			$this->model_activity_activity->addActivity ( 'app_jsonClientoutofcellgraphs', $activity_data2 );
		}
	}
