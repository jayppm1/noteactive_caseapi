...
class ControllerCommonverifyotp extends Controller
{
    ...
    public function index ()
    {
		...
		if($this->session->data['activationkey'] != null && $this->session->data['activationkey'] != ""){
			...
			if ($this->request->post['activationotp'] != NULL && $this->request->post['activationotp'] != "") {
                ...
                $timezone_name = $this->session->data['time_zone'];
                ...
                $data = array(
                        ...
                        'notes_id' => '0',
                        'share_note_otp' => $this->session->data['share_note_otp']
                );
                $getUserdetail1 = $this->model_user_user->getuserOPT($data);
				
				if ($this->request->post['activationotp'] == $getUserdetail1['otp']) {
        ...
        $this->data['resend_otp'] = $this->url->link('common/verifyotp&resend=1', '' . $url3, 'SSL');
		...
		//var_dump($this->session->data['activationkey']);
		
        
        $this->template = $this->config->get('config_template') . '/template/common/verifyotp.php';
        $this->children = array(
                'common/headerlogin',
                'common/footerlogin'
        );
    ...
    public function verifyotp ()
    {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm2()) {
            ...
            $this->data['redirect_url'] = str_replace('&amp;', '&', $this->url->link('common/verifyotp/verifyotp', '' . $url2, 'SSL'));
        }
        ...
        if ($this->request->get['resend'] == "1") {
            ...
            if ($getUserdetail['phone_number'] == null && $getUserdetail['phone_number'] == "") {
                ...
                $this->redirect($this->url->link('common/verifyotp/verifyotp', '' . $url2, 'SSL'));
            } else {
                ...
                if ($getUserdetail1['otp'] != null && $getUserdetail1['otp'] != "") {
					...
					if($getUserdetail1['email'] != null && $getUserdetail1['email'] != ""){
						...
						$edata['message'] = $message;
						$edata['subject'] = $facilities_info['facility'].' | '.'Your verification code';
						$edata['user_email'] = $getUserdetail1['email'];
        ...
        $this->data['action'] = $this->url->link('common/verifyotp/verifyotp', '' . $url2, 'SSL');
        $this->data['resend_otp_page'] = $this->url->link('common/verifyotp/verifyotp&resend=1', '' . $url2, 'SSL');
